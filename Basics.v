...
Fixpoint bin_nat (b : bin) : nat :=
  match b with
    | BO => O
    | BT b' => (mult 2 (bin_nat b'))
    | BI b' => S (mult 2 (bin_nat b'))
  end.

Eval compute in (bin_nat BO).
Eval compute in (bin_nat (BI BO)).
Eval compute in (bin_nat (BT (BI BO))).
Eval compute in (bin_nat (BI (BT (BT (BI BO))))).
Eval compute in (bin_nat (bin_inc (BI (BT (BT (BI BO)))))).
