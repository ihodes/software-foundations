...
0 -- can't construct any instance of baz.
[] *)
...
Fixpoint filter {X:Type} (test: X->bool) (l:list X)
  ...
  match l with
    | nil => nil
    | h :: tl => if test h then h :: (filter test tl) else filter test tl
  end.
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => if evenb n then ble_nat 7 n else false) l.
...
Theorem map_dist : forall (X Y: Type) (f : X -> Y) l (x : X),
...
Fixpoint filter_exp (X:Type) (test: X->bool) (l:list X)
                : (list X) :=
  match l with
    | nil => nil
    | h :: tl => if test h then h :: (filter test tl) else filter_exp X test tl
  end.

Fixpoint map_exp (X Y:Type) (f:X->Y) (l:list X)
             : (list Y) :=
  match l with
  | []     => []
  | h :: t => (f h) :: (map_exp X Y f t)
  end.
...
(** Observe that the type of [fold] is parameterized by _two_ type
 ...
 Yes, if for instance we wanted to check that every element in a list is equal
 to 2. *)
