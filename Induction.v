...
Destruct breaks the proof into cases which may be solved
separately. Induction gives you a base case to prove, and an inductive step to
prove which can rely, in turn, on the "hypothesis" generated by the base case.
...
Theorem mult_S_distribute : forall n m: nat,
  n * S m = n + n * m.
Proof.
  intros n m.
  induction n.
...
Proof.
  ...
  induction m.
    ...
    Case "m = S m'".
      simpl. rewrite <- IHm. rewrite -> mult_S_distribute. reflexivity.
Qed.
...
Theorem mult_S_n : forall n : nat,
  n * S n = n + n * n.
Proof.
  intros n.
  rewrite -> mult_S_distribute. reflexivity.
Qed.
...
Theorem plus_S_dist : forall n : nat,
  S (n + n) = n + S n.
Proof.
  intros n.
  rewrite <- plus_1_l.
  ...
  assert (H: S n = n + 1).
    rewrite -> plus_comm. reflexivity.
  rewrite H. rewrite plus_assoc. rewrite plus_comm. replace (1 + n) with (n + 1). rewrite plus_assoc.
  reflexivity.
Qed.

Theorem binary_commute : forall b : bin,
  bin_nat (bin_inc b) = S (bin_nat b).
Proof.
  intros b.
  induction b.
    Case "b = BO". reflexivity.
    Case "b = BT b". reflexivity.
    Case "b = BI b". simpl. rewrite IHb. simpl.
      rewrite plus_0_r. rewrite plus_S_dist.
      reflexivity.
Qed.
...
Fixpoint nat_bin (n : nat) : bin :=
  match n with
    | O => BO
    | S n' => bin_inc (nat_bin n')
  end.

Theorem nat_bin_nat_inversion : forall n : nat,
  bin_nat (nat_bin n) = n.
Proof.
  intros n.
  induction n.
    Case "n = 0". reflexivity.
    Case "n = S n". simpl. rewrite binary_commute. rewrite IHn. reflexivity.
Qed.

Example nat_bin_3: (nat_bin 3) = BI (BI BO).
Proof. reflexivity. Qed.
Example nat_bin_10: (bin_nat (nat_bin 10)) = 10.
Proof. reflexivity. Qed.

Eval compute in (bin_nat (BT (BT (BI BO)))).
Eval compute in (bin_nat (BI (BT (BI BO)))).
Eval compute in (bin_nat (BT (BT (BI (BI BO))))).

(* We cannot convert to the same binary number you started with because there
   are multiple representations for a binary number. Any binary number can start
   with an arbitrary number of BT (BT (.. BO)), for example.  *)
